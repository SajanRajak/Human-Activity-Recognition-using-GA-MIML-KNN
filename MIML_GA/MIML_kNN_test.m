function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels,te_time]=MIML_kNN_test(train_ModBg,train_tarMod,test_cv,test_tarCV,num_ref,num_citer,Weights)
%MIML_kNN_test tests a lazy multi-instance multi-label learner.
%
%    Syntax
%
%       [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels,te_time]=MIML_kNN_test(train_bags,train_target,test_cv,test_tarCV,num_ref,num_citer,Weights)
%
%    Description
%
%       MIML_kNN_test takes,
...
%           test_cv        - An M2x1 cell, the ith test bag is stored in test_cv{i,1}
%           test_tarCV      - A QxM2 array, if the ith test bag belongs to the jth class, test_tarCV(j,i) equals +1, otherwise test_tarCV(j,i) equals -1
...
%           te_time          - The time spent in testing
    ...
    num_test=size(test_cv,1);
    num_inst=num_train+num_test;
    bags=[train_ModBg;test_cv];
...
%Evaluation
    ...
    HammingLoss=Hamming_loss(Pre_Labels,test_tarCV);
    RankingLoss=Ranking_loss(Outputs,test_tarCV);
    OneError=One_error(Outputs,test_tarCV);
    Coverage=coverage(Outputs,test_tarCV);
    Average_Precision=Average_precision(Outputs,test_tarCV);
