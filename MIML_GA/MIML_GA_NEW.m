...
%% Main Loop (Evolution Loop)
    ...
    for it=1:MaxIt
        ...
        for k=1:nc/2
            ...
            % Evaluate Offsprings
            [popc(k,1).Cost,popc(k,1).z,popc(k,1).Predicted_Labels,popc(k,1).Actual_Test_Labels]=feature_selection_new(popc(k,1).Position,data_a,data_b );
            [popc(k,2).Cost,popc(k,2).z,popc(k,2).Predicted_Labels,popc(k,2).Actual_Test_Labels]=feature_selection_new(popc(k,2).Position,data_a,data_b );
        ...
        for k=1:nm
            ...
            % Evaluate Mutant
            [popm(k).Cost,popm(k).z,popm(k).Predicted_Labels,popm(k).Actual_Test_Labels]=feature_selection_new(popm(k).Position,data_a,data_b );
